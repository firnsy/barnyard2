...
//	optp=otn_tmp->ds_list[PLUGIN_FWSAM];
	...
	{	if( ((optp->how==FWSAM_HOW_THIS)?	/* if blocking mode SERVICE, check for src and dst	  */
		( lastbsip[i]==record->sip && lastbdip[i]==record->dip &&lastbproto[i]==record->protocol &&
				  (needports? /* check port only of TCP or UDP */
/*					((optp->who==FWSAM_WHO_SRC)?(lastbsp[i]==record->sp):(lastbdp[i]==record->dp)):TRUE) ): */
					lastbdp[i]==record->dp:TRUE) ):
				((optp->who==FWSAM_WHO_SRC)?(lastbsip[i]==record->sip):(lastbdip[i]==record->dip))) && /* otherwise if we block source, only compare source. Same for dest. */
		...
		{	if(++lastbpointer>=FWSAM_REPET_BLOCKS)		/* increase repetitive check pointer */
				lastbpointer=0;
			lastbsip[lastbpointer]=record->sip;		/* and note packet details */
			lastbdip[lastbpointer]=record->dip;
			lastbduration[lastbpointer]=optp->duration;	
			lastbmode[lastbpointer]=optp->how|optp->who|optp->loglevel; 
			lastbproto[lastbpointer]=record->protocol;			
			if(needports)
			{	lastbsp[lastbpointer]=record->sp;					/* set ports if TCP or UDP */
				lastbdp[lastbpointer]=record->dp;
			}
