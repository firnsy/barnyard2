...
{	_TwoFish_ResetCBC(tfdata);							/* reset CBC flag. */
	tfdata->output=(uint8_t *)out;							/* output straight into output buffer. */
...
{	tfdata->output=(uint8_t *)outp;				/* (do we really need a function for this?) */
}
...
void _TwoFish_BinHex(uint8_t *buf,unsigned long len,bool bintohex)
{	uint8_t *pi,*po,c;
...
{	unsigned long ilen,olen;
	...
	{	if(*out==NULL)									/* if OUT points to a NULL pointer... */
		...
		if(*out!=NULL)
		{	tfdata->output=(uint8_t *)*out;							/* set output buffer. */
			...
			_TwoFish_BlockCrypt((uint8_t *)&(tfdata->header),(uint8_t *)*out,olen,FALSE,tfdata); /* encrypt first block (without flush on 16 byte boundary). */
			olen+=_TwoFish_CryptRawCBC(in,*out+TwoFish_BLOCK_SIZE,ilen,FALSE,tfdata);	/* and encrypt the rest (we do not reset the CBC flag). */
			if(binhex)									/* if binhex... */
			{	_TwoFish_BinHex((uint8_t *)*out,olen,TRUE);		/* ...convert output to binhex... */
				olen*=2;								/* ...and size twice as large. */
			}
			tfdata->output=(uint8_t *)*out;
			return olen;
		}
	}
	return 0;
...
{	unsigned long ilen,elen,olen;
	...
	{	if(*out==NULL)									/* if OUT points to a NULL pointer... */
		...
		if(*out!=NULL)
		{	if(binhex)									/* if binhex... */
			{	_TwoFish_BinHex((uint8_t *)in,ilen,FALSE);		/* ...convert input to values... */
 ...
 * @return none
 */
void _TwoFish_BlockCrypt(uint8_t *in,uint8_t *out,unsigned long size,int decrypt,TWOFISH *tfdata)
...
void _TwoFish_FlushOutput(uint8_t *b,unsigned long len,TWOFISH *tfdata)
{	unsigned long i;
